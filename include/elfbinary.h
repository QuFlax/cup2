...
typedef struct _elfsegment {
  Elf_Phdr _;
  span span;
  void* sections; // //std::vector<Section*>
...
typedef struct _elfsection {
  Elf_Shdr _;
  void* segments; //std::vector<Segment*>;
  span span;
  int is_frame;
  //DataHandler::Handler* datahandler_ = nullptr;
  //std::vector<uint8_t> content_c_;
...
void shift_segments(elfbinary* b, uint64_t from, uint64_t shift);

void shift_desr(elfbinary* b, uint64_t from, uint64_t shift);

uint64_t relocate_phdr_table_pie(elfbinary* b);
uint64_t relocate_phdr_table(elfbinary* b, enum PHDR_RELOC r);

/*
uint64_t relocate_phdr_table(ELF_Binary* b, PHDR_RELOC r) {
  switch (r) {
    case PHDR_RELOC::PIE_SHIFT:
      return relocate_phdr_table_pie(b);
    case PHDR_RELOC::SEGMENT_GAP:
      return relocate_phdr_table_v1(b);
    case PHDR_RELOC::BSS_END:
      return relocate_phdr_table_v2(b);
    case PHDR_RELOC::BINARY_END:
      return relocate_phdr_table_v3(b);
    case PHDR_RELOC::AUTO:
    default:
      return relocate_phdr_table_auto(b);
  }
}
*/
