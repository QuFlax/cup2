...
static Node getNode(CUPModule *cc) {
  if (cc->it >= cc->end) {
...
size_t codegen_expr(CUPModule *buf, X64Reg treg, X64Reg sreg) {
  CTType ntype = getNode(buf).type;
  size_t value = 0;
  switch (ntype) {
    case N_BLOCK:
    case T_COMMA:
      return getNode(buf).value;
    case T_NUMBER: {
      emit_mov_reg_imm64(buf, treg, getNode(buf).value);
      return 0;
    }
    case N_VARIABLE: {
    ...
    case T_EQ: {
      size_t i = codegen_expr(buf, RBX, RBX);
      if (i) {
    ...
    case T_LESSEQ: {
      ...
      value = codegen_expr(buf, treg, sreg);
      if (value) {
      ...
      value = codegen_expr(buf, temp_reg, temp_reg);
      if (value) {
    ...
    case T_CALL: {
      ...
      // const CUPType *type = defType(buf->state, buf->it, NULL);
      const CUPType *type = NULL;
      defType(buf->state, buf->it, &type);
      value = codegen_expr(buf, RAX, RAX);
      if (value) {
        cup_error("N_CALL 1");
        exit(1);
      }
...
#if DEBUG_LOG
      printf("call = %d, %i\n", count, type->elements[0]->realtype);
#endif
      if (count > 6) {
      ...
      for (value = 0; value < count; value++) {
        if (codegen_expr(buf, sysv_arg_regs[value], sysv_arg_regs[value])) {
    ...
    break;*/
      ...
      printf("--: Save --\n");
#endif
      size_t i = codegen_expr(buf, RAX, RAX);
#if DEBUG_LOG
      printf("--: RESTORE --\n");
#endif
      buf->size = size;
      ...
      size_t arg_count = 0;
      while (var->type->elements[arg_count + 1]) arg_count++;
      codegen_func(buf, &var->value, arg_count);
      return 0;
    }
    case T_RETURN: {
      size_t i = codegen_expr(buf, RAX, RAX);
...
void codegen_func(CUPModule* buf, size_t *value, size_t i) {
...
#endif
  if (i) {
    size_t i_save = codegen_expr(buf, RAX, ANY);
    if (i != i_save) {
      exit(17);
    }
  }
  for (size_t j = 0; j < i; j++) {
    codegen_expr(buf, RAX, ANY);
  ...
  i = codegen_expr(buf, RETURN_REG, ANY); // get codegen left count
  for (size_t j = 0; j < i; j++)
    codegen_expr(buf, RETURN_REG, ANY);
