...
static void symbol_cb(CUPState *state, void *ctx, const char *name, const CVariable var) {
  (void)state;
  char tname[256];
  cup_type_snname(tname, sizeof(tname), var.type);
  fprintf((FILE*)ctx, "%s[%ld], type= %s, value= %ld\n", name, var.scope, tname, var.value);
}

static void cup_write_bytecode(CUPState *state, const NRange range) {
  if (state == NULL)
    return;
  const char *configured = state->bytecode_path;
  const char *filename = configured;
  FILE *f = fopen(filename, "w");
  if (f == NULL) {
    cup_errorf("Could not open bytecode output '%s'", filename);
    return;
  }
  cup_list_symbols(state, f, symbol_cb);
  
  for (size_t i = 0; i < state->data_size; i++)
    fputc(*getData(state, i), f);
    //fprintf(f, "%hhx", *getData(state, i));
  fputc('\n', f);
  //printNodes(f, range, state);
  ...
  fclose(f);
  state->bytecode_path = NULL;
}

void codegen(CUPState *state, CUPModule *m, NRange range) {
  size_t init = 0;
  //const NRange range = state->range;
  state->vector_ = (CVector){};
  state->scope = 0;
  codegen_func(state, m, &range, &init, 0);
  //m->range = range;
...
CUPModule *parse(CUPState *state, const char *buf, const char *file) {
  ...
  int def = defType(state, &nodes);
  NRange range = (NRange){nodes.data, (Node *)((char *)nodes.data + nodes.size)};
  if (state->bytecode_path)
    cup_write_bytecode(state, range);
  if (def == 0) {
    codegen(state, &m->module, range);
  }
  cup_free(nodes.data);
...
void cup_delete(CUPState *state) {
  ...
  while (m) {
    ...
    cup_free(m->module.objrefs.data);
    cup_free(m);
    m = next;
  }
